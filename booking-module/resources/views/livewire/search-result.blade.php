    ...
    <div class=" relative flex flex-wrap w-full">
        ...
        <div class="xs:w-8/12 w-full border-r border-gray-100">
            <div id="fr-left-side">
                ...
                @php $legArrayIndex = 0 @endphp
                    @foreach ($search_result['leg_array'] as $leg)
                        
                    <div class="p-4 border-b border-gray-100">
                        <p class="mb-2 font-medium leading-5 text-gray-500">
                            <time>{{$leg[0]['leg_origin_date']}}</time>
                        </p>
                        <div class=" relative">
                            <div class="flex">
                                <div class=" relative flex items-center flex-shrink-0 mr-2">
                                    ...
                                    <p class=" m-0 text-sm font-normal leading-5 text-left text-gray-700">
                                        <strong class=" font-medium">
                                            <time>{{$leg[0]['leg_origin_time']}}</time><span class="sm:ml-2 ml-3 font-medium">{{$leg[0]['leg_origin_city']}}</span>
                                            <span class="block sm:inline mt-0.5 text-gray-500 leading-5 sm:mt-0 sm:ml:4 sm:text-sm ">{{$leg[0]['leg_origin_IATA_code']}}</span>
                                        </strong>
                                    </p>
                                </div>
                                        ...
                                        <div class="mt-0 mb-1 ml-0 mr-1">
                                            <div class="rounded-xl box-border inline-flex items-center justify-center flex-auto min-h-full px-2 py-0 text-xs font-medium leading-4 text-blue-500 bg-blue-100 border border-blue-200">
                                                <div class=" px-0 py-1 leading-none">
                                                        ...
                                                        @if ($legArrayIndex === 0)
                                                            @php $iterate = $search_result['leg_array'][0]['outbound_flightCount']@endphp
                                                            <time>
                                                                @if (floor(($search_result['leg_duration_outbound']/60)/60) > 0)
                                                                {{ floor(($search_result['leg_duration_outbound']/60)/60) }}h
                                                                @endif

                                                                @if ((($search_result['leg_duration_outbound'])/60 % 60) > 0)
                                                                {{ (($search_result['leg_duration_outbound'])/60 % 60) }}m
                                                                @endif
                                                            </time>
                                                        @else
                                                            @php $iterate = $search_result['leg_array'][1]['inbound_flightCount']@endphp
                                                            <time>
                                                                @if (floor(($search_result['leg_duration_inbound']/60)/60) > 0)
                                                                {{ floor(($search_result['leg_duration_inbound']/60)/60) }}h
                                                                @endif

                                                                @if ((($search_result['leg_duration_inbound'])/60 % 60) > 0)
                                                                {{ (($search_result['leg_duration_inbound'])/60 % 60) }}m
                                                                @endif
                                                            </time>
                                                        @endif

                                                        <!-- $search_result['duration_departure'] -->
                            ...
                            <div class=" flex">
                                ...
                                <p class=" m-0 text-sm font-normal leading-5 text-left text-gray-700">
                                    <strong class=" font-medium">
                                        <time>{{$leg[1]['leg_destination_time']}}</time><span class=" sm:ml-2 ml-3">{{$leg[1]['leg_destination_city']}}</span>
                                        <span class="block sm:inline mt-0.5 text-gray-500 leading-5 sm:mt-0 sm:ml:4 sm:text-sm ">{{$leg[1]['leg_destination_IATA_code']}}</span>
                                    </strong>
                                </p>
                        ...
                        @if (($leg[1]['leg_destination_date']) > ($leg[0]['leg_origin_date']))
                        <p class="mt-2 font-medium leading-5 text-gray-500">
                            <time>{{ $leg[1]['leg_destination_date'] }}</time>
                        </p>
                        @endif
                    </div>
                    @php $legArrayIndex++ @endphp
                    @endforeach
                ...
                <div class=" sm:p-4 sm:border-b-0 flex flex-wrap justify-between px-4 py-2 pb-2 border-b-0 border-gray-200 border-solid">
                    ...
                    <div class=" inline-flex flex-row flex-wrap flex-grow">
                        @foreach ($search_result['cabin_array'] as $cabin)
                        <div class="mt-0 mb-1 ml-0 mr-1">
                        ...
                        <!-- baggage - hand           -->
                        @if (isset($search_result['hand_luggage_limit']))
                        <div class="mt-0 mb-1 ml-0 mr-1">
                            <div class="rounded-xl box-border inline-flex items-center justify-center flex-auto min-h-full px-2 py-0 text-xs font-medium leading-4 text-gray-500 bg-gray-100 border border-gray-200">
                                ...
                                <div class=" px-0 py-1 leading-none">
                                    <time>{{$search_result['hand_luggage_limit'] ?? ''}} kg</time>
                                </div>
...
<div class="xs:hidden xs:ml-auto sm:pl-4 xs:whitespace-nowrap inline-flex">
    ...
    <button   
        ...
        wire:click="$emit('flightSelected',{{ json_encode($search_result) }})" 
        class="xs:ml-0 xs:flex-shrink xs:w-full hover:bg-indigo-800 inline-flex items-center justify-center flex-grow py-2 ml-2 text-xs font-semibold text-white bg-indigo-700 rounded">
        Book
    </button>
